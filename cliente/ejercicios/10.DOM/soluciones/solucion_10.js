...
el texto abrir/cerrar. Al pulsarlo se deben ocultar o mostrar los botones 
restantes, pero el de abrir/cerrrar debe quedar visible.

El valor de la propiedad display de los botones es inline-block. 

La barra tiene en el atributo class el valor "botones" y cada botón tiene como 
clase "boton".
...
// Ej 10: carrusel con JS
function cambiaImg(){
  // determinamos el máximo posible de data-img a partir de la cantidad de elementos
  // del array
  const MAX_DATA_IMG = pokeImgs.length - 1;
  // seleccionamos el div padre:
  const div = document.getElementsByClassName("carrusel")[0]; // primer elemento 
                      // del array de elementos de clase carrusel (solo hay uno)
                      // devuelve undefined si no hay elementos en el array
  // convertimos el valor de data-img de string a entero:
  let valorActual = Number.parseInt(div.getAttribute("data-img"));
  // valores posibles: 0 -> 1 -> 2 -> 3 -> 0...
  // Nota: se podría conseguir lo mismo con el operador % 4 (cicla del 0 al 3)
  // o podemos hacer un bucle:
  if(valorActual == MAX_DATA_IMG){
    div.setAttribute("data-img", `0`); // reiniciamos a la primera img
  } else {
    div.setAttribute("data-img", `${valorActual += 1}`); // incrementamos 1 en 
                                        // data-img y lo guardamos como string
  }
  // usamos el campo data-img para seleccionar la img correcta:
  const imgNuevaSrc = pokeImgs[Number.parseInt(div.getAttribute("data-img"))] 
  // colocamos el src en la img:
  const img = div.querySelector("img"); // seleccionamos la img en el div
  img.src = imgNuevaSrc;
  console.log(div)
}
// Ej 10: toggle de botones
function toggle(){
  // array con todos los botones:
  const botones = document.getElementsByClassName("boton");
  // para ignorar el primero "Abrir / Cerrar" empezamos a iterar en el 1
  if(!botonesColapsados){
    for(let i = 1; i < botones.length; i++){
      const boton = botones[i]; // se salta el primer botón
    ...
    for(let i = 1; i < botones.length; i++){
      ...
      // Nota: si se usa la opción de cambiar display, es posible que haya
      // que reconsiderar cambios de alineamiento porque Abrir / Cerrar se "desplaza"
    }
  }
  // toggle del booleano:
  botonesColapsados = !botonesColapsados; // false <-> true
}
